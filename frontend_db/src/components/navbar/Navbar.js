...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const storedCartItems = Object.keys(localStorage)
      ...
      .map(key => ({
        id: key.split('-')[1],
        quantity: parseInt(localStorage.getItem(key))
      }));
    setCartItems(storedCartItems);
  }, [localStorage]);

  const calculateSubtotal = () => {
    // Assuming you have product details including prices in local storage or you can fetch them
    // Here we will just return a mocked subtotal for demonstration purposes
    return cartItems.reduce((total, item) => total+ (item.quantity), 0); // Assuming each product costs $20
  };
    ...
    <div className="navbar bg-base-100 bg-slate-900 text-white flex flex-row items-center justify-between">
      ...
      <div>
        ...
        <div className="dropdown dropdown-end">
          <div tabIndex={0} role="button" className="btn btn-ghost btn-circle carts">
            <div className="indicator">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
              <span className="badge badge-sm indicator-item">{calculateSubtotal()}</span>
            </div>
          </div>
          <div tabIndex={0} className="mt-3 z-[1] card card-compact dropdown-content w-52 bg-base-100 shadow">
            <div className="card-body text-black">
              <span className="font-bold text-lg">{calculateSubtotal()}</span>
              <span className="text-info text-black">Subtotal: ${calculateSubtotal()}</span>
        ...
        <div className="dropdown dropdown-end">
          <div tabIndex={0} role="button" className="btn btn-ghost btn-circle avatar">
            <div className="w-10 rounded-full">
              <i className="ri-account-circle-line text-3xl font-light"></i>
            </div>
          </div>
